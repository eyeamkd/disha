...
export class NewDspaceForm extends Component {
    ...
    getUserDetails = () => {
        ...
        if(currentUserInfo) {
            this.setState({isAuthenticated : JSON.parse(currentUserInfo).isAuthenticated })
        }
        else {
    ...
    dSpaceExists() {
        debugger;
        let id = this.titleToId(this.state.dSpaceTitle)
        this.setState({dSpaceId: id})
        let ids = this.fetchExistingId(id)
        console.log("ID exists: ", ids.empty)
        return !ids.empty
    }

    async fetchExistingId(id) {
        let dSpaceData = database.collection('d-spaces').where("id", "==", id).get().then(snapshot => {
            if(snapshot.empty) {
                ...
                this.setState({ dSpaceExists: true });
            }
        });
        debugger;
        return dSpaceData
        ...
        if (this.state.dSpaceCreatedSuccessfully) {
            ...
            return (
                <Container className={this.state.isAuthenticated ? '' : 'display-inactive'}>
                    ...
                    {this.state.dSpaceExists &&
                        <FormHelperText error={true}>D-Space with this name already exists. Choose another name</FormHelperText>
                    }
