...
export class DspacePosts extends Component {
  ...
  getPosts = () => {
    ...
    let query = postsData
      .where("dSpaces", "array-contains", this.props.dSpace.title)
      .get()
      .then((snapshot) => {
        if (snapshot.empty) {
    ...
    if (this.state.filterValue === "None") {
      return (
        <Post
          ...
          postedByUser={
            post.authorRollNumber === this.state.currentUser.rollNumber
          }
      ...
      if (!this.state.noPostsYet) {
        return (
          <div>
            ...
            >
              <MenuItem onClick={() => this.handleClose("Events")}>
                Events
              </MenuItem>
              <MenuItem onClick={() => this.handleClose("Internship")}>
                Internship
              </MenuItem>
              <MenuItem onClick={() => this.handleClose("Project")}>
                Project
              </MenuItem>
